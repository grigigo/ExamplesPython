...
command1 = command1.split()[len(command1.split()) - 1].split(',')
command2 = command2.split()[len(command2.split()) - 1].split(',')
result = []
for elem in command2:
    if not result.count(elem) and command1.count(elem):
        result.append(elem)
